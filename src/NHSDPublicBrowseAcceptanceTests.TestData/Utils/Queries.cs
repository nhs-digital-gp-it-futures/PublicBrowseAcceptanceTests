namespace NHSDPublicBrowseAcceptanceTests.TestData.Utils
{
     internal static class Queries
    {
        ...
        internal const string GetSingleSolution = "SELECT Sol.[Id],Sol.[Name],Sol.[LastUpdated],Det.[AboutUrl],Det.[Summary],Org.[Name] as SupplierName FROM [buyingcatalogue].[dbo].[Solution] as Sol LEFT JOIN [buyingcatalogue].[dbo].[SolutionDetail] as Det ON Det.[Id] = Sol.[SolutionDetailId] LEFT JOIN [buyingcatalogue].[dbo].[Organisation] as Org ON Sol.OrganisationId = Org.Id WHERE Sol.Id = @solutionId";
